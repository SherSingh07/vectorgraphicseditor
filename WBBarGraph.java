...
class WBBarGraph extends WBShape{
	String s;
	int max = 0;
	int hp, bw;
	int items;
    ...
    Color col;
	public WBBarGraph (int x, int y, int w, int h, Color col, String s){
	...
	public void draw(Graphics g) {
		...
		if(max<=100000)    { maxBH = ((max/10000)+1);}
		...
		for (int i=0;i<values.length ;i++)		{
			int next = Integer.parseInt((String)values[i]);
		 g.setColor(col);
		 g.fillRect(x+((3*i)+1)*w/(3*values.length), (y+h)-((next*hp)/max), w/(3*values.length), ((next*hp)/max));

		 g.setColor(Color.BLACK);
		 g.drawRect(x+((3*i)+1)*w/(3*values.length), (y+h)-((next*hp)/max), w/(3*values.length), ((next*hp)/max));
