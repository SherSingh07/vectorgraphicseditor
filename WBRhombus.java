...
class WBRhombus extends WBShape{
	
	int size;
	WBRhombus(int x, int y, int w, int h,int size, Color col, boolean draw){
		super(x, y, w, h, col, draw);
		this.size = size;
	}
	
	WBRhombus(WBRhombus wbs){
		super(wbs.x, wbs.y, wbs.w, wbs.h, wbs.col, wbs.draw);
		this.size = wbs.size;
	}

	public String toXml(){ 
		String xml = "<WBRhombus>";  
        xml += "<x>" + x + "</x>";
        xml += "<y>" + y + "</y>"; 
        xml += "<w>" + w + "</w>";
        xml += "<h>" + h + "</h>";
	...
	public void draw(Graphics g){
		...
		if(draw){
			...
			g2d.drawPolygon(ax, ay, 4);	
		}
		else {
			int[]ax1 = {x + (w/2), x + w, x + (w/2), x}; 
			int[]ay1 = {y, y+h/2, y+h, y+h/2};
