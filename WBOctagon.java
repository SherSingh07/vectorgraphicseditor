...
class WBOctagon extends WBShape{

	int size;

	public WBOctagon (int x, int y, int w, int h,int size, Color col, boolean draw){
		super(x, y, w, h, col, draw);
		this.size = size;
	}

	WBOctagon(WBOctagon wbs){
		super(wbs.x, wbs.y, wbs.w, wbs.h, wbs.col, wbs.draw);
		this.size = wbs.size;
	}

	public String toXml(){ 
		String xml = "<WBOctagon>";  
        xml += "<x>" + x + "</x>";
        xml += "<y>" + y + "</y>"; 
        xml += "<w>" + w + "</w>";
        xml += "<h>" + h + "</h>";
	...
	public void draw(Graphics g){
		...
		if(draw){
			int ax[] = {x+w/4, x+(3*w)/4, x+w, x+w, x+(3*w)/4, x+w/4, x, x};
			int ay[] = {y, y, y+h/4, y+(3*h)/4, y+h, y+h, y+(3*h)/4, y+h/4};
